...
namespace WPF3DTest
{
    ...
    {
        // Points , ColorValues , SurfaceBrush 为ModelVisual3D自身所的属性
        // 添加依附，主要是为了方便函数调用，Point3D[,]表明函数的返回类型
        ...
        private Model3D CreateModel()
        {
            ...
            // MaterialHelper.CreateMaterial(brush, null, Brushes.Gray, 1.0, 200);
            ...
            for (int i = 0; i < Points.GetLength(0); i++)
            {
                ...
                axesMeshBuilder.AddTube(path, 0.1, 9, true);
            }
        ...
        public Point[,] GetTexCoords()
        {
            ...
            for (int i = 0; i < Points.GetLength(0); i++)
                for (int j = 0; j < Points.GetLength(1); j++)
                {
                    double axis = Math.Sqrt(Math.Pow(Points[i, j].X , 2) + Math.Pow(Points[i, j].Y, 2) + Math.Pow(Points[i, j].Z, 2));
                    maxAxis = Math.Max(maxAxis, axis);
                    minAxis = Math.Min(minAxis, axis);
                }

            Point[,] texcoords = new Point[Points.GetLength(0), Points.GetLength(1)];
            for (int i = 0; i < Points.GetLength(0); i++)
                for (int j = 0; j < Points.GetLength(1); j++)
                {
                    // 轴线，越长才越红
                    double axis = Math.Sqrt(Math.Pow(Points[i, j].X, 2) + Math.Pow(Points[i, j].Y, 2) + Math.Pow(Points[i, j].Z, 2));
                    double u = axis / maxAxis;
                    if (u > 0.9)
                    {
